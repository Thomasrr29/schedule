...
type Fila = [string, Dia, string, string, string, string, TipoBloque];

const BLOQUES: Fila[] = [
  // Lunes: yo salgo 12:00 y ella entra 12:00 -> cruce en la puerta (corto).
  ["Estadística", "lun", "12:00", "14:00", "robledo", "N-204", "OCUPADO"],
  // Martes: misma hora que mi clase pero en otra sede -> no cuenta.
  ["Física II", "mar", "14:00", "16:00", "fraternidad", "M-203", "OCUPADO"],
  // Miércoles: mi bloque DISPONIBLE no se resta -> encuentro largo al salir ella.
  ["Redes", "mie", "14:00", "16:00", "robledo", "C-308", "OCUPADO"],
  // Jueves: ella sola en el campus, yo no caigo -> nada.
  ["Ética", "jue", "08:00", "10:00", "robledo", "C-201", "OCUPADO"],
  // Viernes: yo salgo 09:00, ella entra 09:00, misma sede -> corto.
  ["Cálculo II", "vie", "09:00", "11:00", "fraternidad", "N-310", "OCUPADO"],
];
...
async function main() {
  ...
  const sedes = new Set((await prisma.sede.findMany({ select: { id: true } })).map((s) => s.id));
  const faltan = [...new Set(BLOQUES.map((b) => b[4]))].filter((id) => !sedes.has(id));
  if (faltan.length) throw new Error(`Faltan sedes en la base (${faltan.join(", ")}). Corré: npx prisma db seed`);
    ...
    data: {
      ...
      schedules: {
        create: {
          ...
          blocks: {
            create: BLOQUES.map(([titulo, dia, horaInicio, horaFin, sedeId, aula, tipo]) => ({
              titulo, dia, horaInicio, horaFin, sedeId, aula, tipo, origen: "manual" as const,
            })),
          },
        },
      },
