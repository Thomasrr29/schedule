...
import { calcularCoincidencias, type Coincidencia } from "@/lib/matching";
...
export async function coincidenciasDe(userId: string): Promise<AmigoConCoincidencias[]> {
  ...
  const horarios = await prisma.schedule.findMany({
    ...
    include: {
      blocks: { select: { titulo: true, dia: true, horaInicio: true, horaFin: true, sede: true, tipo: true } },
  ...
  return horarios
    ...
    .map((h) => ({ amigo: h.user, coincidencias: calcularCoincidencias(mio.blocks, h.blocks) }));
}
